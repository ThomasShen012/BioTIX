...
def home():
    ...
    if user_json:
        ...
        recent_event = list(
            mydb.events.find(
                {"time": {"$gte": current_datetime}}, {"_id": 1, "title": 1, "time": 1}
            )
            .sort("time", 1)
            .limit(6)
        )
        recent_sale = list(
            mydb.events.find(
                {"time": {"$gte": current_datetime}},
                {"_id": 1, "title": 1, "ticket_time": 1},
            )
            .sort("ticket_time", 1)
            .limit(6)
        )
        ...
        return render_template(
            "home.html", recent_event=recent_event, recent_sale=recent_sale
        )
...
def video_feed():
    return Response(
        gen(video_stream), mimetype="multipart/x-mixed-replace; boundary=frame"
    )
...
def purchase_details():
    ...
    return render_template("order_detail.html", orders=list(orders))  # 將查詢結果轉為列表
